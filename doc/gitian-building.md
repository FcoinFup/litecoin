...
*Setup instructions for a Gitian build of Deliverycoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Deliverycoin
...
- [Building Deliverycoin](#building-deliverycoin)
...
Clone the git repositories for deliverycoin and Gitian and then checkout the deliverycoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the deliverycoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for Gitian to work offline.

Building Deliverycoin
----------------

To build Deliverycoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the deliverycoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/deliverycoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/deliverycoin-project/deliverycoin
...
For example:
```bash
URL=https://github.com/user/deliverycoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit deliverycoin=${COMMIT} --url deliverycoin=${URL} ../deliverycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit deliverycoin=${COMMIT} --url deliverycoin=${URL} ../deliverycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit deliverycoin=${COMMIT} --url deliverycoin=${URL} ../deliverycoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/deliverycoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/deliverycoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/deliverycoin-build.assert
```
...
[deliverycoin/gitian.sigs](https://github.com/deliverycoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
